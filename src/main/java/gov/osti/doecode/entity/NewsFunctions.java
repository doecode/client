...
public class NewsFunctions {
   ...
   static LocalDateTime getEndPublicationDateTime(LocalDateTime start_pub_date, ObjectNode obj_filters) {
      ...
      if (obj_filters.findPath("has_publication_date_year").asBoolean(false)) {
         return_data = return_data.withYear(start_pub_date.getYear()).withMonth(12).withDayOfMonth(31).withHour(23)
   ...
   public static ObjectNode getFilterParamList(ArrayNode article_type_list, LocalDateTime publication_date_start,
      ...
      if (date_filter_obj != null) {
         ...
         String pub_month_display = StringUtils
               .capitalize(publication_date_start.getMonth().getDisplayName(TextStyle.FULL, Locale.getDefault()));
         String pub_month_val = StringUtils.leftPad(Integer.toString(publication_date_start.getMonthValue()), 2, "0");
         ...
         if (date_filter_obj.findPath("has_publication_month_day_year").asBoolean(false)) {
            ...
            filter_list.add(NewsFunctions.makeNewsFilterNode(pub_month_display + " " + pub_day + ", " + pub_year,
         ...
         if (date_filter_obj.findPath("has_publication_minute").asBoolean(false)) {
            ...
            filter_list.add(
                  NewsFunctions.makeNewsFilterNode(publication_date_start.format(NewsFunctions.NEWS_HOUR_MINUTE_AMPM),
      ...
      try {
         ...
         if (publication_date_filters != null) {
            ...
            if (publication_date_filters.get("has_publication_date_year").asBoolean(false)) {
               ...
               for (LocalDateTime ldt : publication_date_list) {
                  String monthyear_key = StringUtils.leftPad(Integer.toString(ldt.getMonthValue()), 2, "0")
               ...
               for (String key : pub_monthyears.keySet()) {
                  ...
                  // 2018", for example
                  String month_display = Month.of(month).getDisplayName(TextStyle.FULL, Locale.getDefault());
            ...
            if (publication_date_filters.get("has_publication_month_year").asBoolean(false)) {
               ...
               for (LocalDateTime ldt : publication_date_list) {
                  ...
                  String key = StringUtils.leftPad(Integer.toString(ldt.getMonthValue()), 2, "0")
                        + StringUtils.leftPad(Integer.toString(ldt.getDayOfMonth()), 2, "0") + ldt.getYear();
         ...
         if (featured_content_words.length > SearchFunctions.MAX_WORD_IN_FEATURED_ARTICLE) {
            featured_article.put("content",
                  StringUtils.join(
                        ArrayUtils.subarray(featured_content_words, 0, SearchFunctions.MAX_WORD_IN_FEATURED_ARTICLE),
                        " ") + "...");
            featured_article.put("content_over_limit", true);
         }
         // Grab the first article type, and say that it will be the type we show
         ObjectNode featured_article_first_type = (ObjectNode) ((ArrayNode) featured_article.get("article_types"))
               .get(0);
   ...
   private static String makeNewsURLParams(ArrayList<String> article_types_join_list,
      ...
      if (article_types_join_list.size() > 0) {
         ...
         return_data += ("&fq=" + URLEncoder.encode(
               "sm_vid_Article_Type:(\"" + StringUtils.join(article_types_join_list, "\" OR \"") + "\")", "UTF-8"));
      }
      if (use_pubdate) {
         // Make publication date range
         return_data += ("&fq="
               + URLEncoder.encode("ds_created:[\"" + publication_date_start.format(SearchFunctions.SOLR_DATE_ONLY)
                     + "T" + publication_date_start.format(SearchFunctions.SOLR_TIME_ONLY) + "Z\" TO \""
                     + publication_date_end.format(SearchFunctions.SOLR_DATE_ONLY) + "T"
                     + publication_date_end.format(SearchFunctions.SOLR_TIME_ONLY) + "Z\"]", "UTF-8"));
      }
      return return_data;
   }
}

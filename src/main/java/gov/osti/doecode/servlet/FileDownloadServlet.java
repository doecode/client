...
public class FileDownloadServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String remaining = StringUtils.substringAfterLast(request.getRequestURI(), Init.app_name + "/download-repository/");
        log.debug(remaining);
        if (remaining.startsWith("container-download/")) {
            ...
            if (!StringUtils.isNumeric(code_id)) {
                response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Your request contained an invalid code id.");
                return;
            }
            // Get the json for the file
            ObjectNode code_id_data = JsonUtils.MAPPER.createObjectNode();
            ...
            if (raw_biblio_call.has("metadata")) {
                code_id_data = (ObjectNode) raw_biblio_call.get("metadata");
            }

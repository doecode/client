    ...
    return (
      <div>
        {this.props.items.map((row, index) => <div key={this.props.prefix + index}>
          ...
          {row.first_name && row.middle_name && <span>&nbsp;</span>}{row.middle_name && <span>{row.middle_name}</span>}
          {(row.first_name || row.middle_name) && row.last_name && <span>&nbsp;</span>}{row.last_name && <span>{row.last_name}</span>}
          {(row.first_name || row.middle_name || row.last_name) && row.affiliations.length > 0 && <span>&nbsp;</span>}{row.affiliations.length > 0 && <span>
            &nbsp;&nbsp;(
